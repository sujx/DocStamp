...
# ── Build ──────────────────────────────────────────────────────────
# --build-arg BUST_FRONTEND=$(date +%s) only invalidates the npm run build
# layer (generating fresh JS chunk hashes) while keeping npm ci, apt, pip
# layers cached.  This avoids the 5-minute full rebuild of --no-cache.
...
echo ""
log "ECS 上部署 (Lite 3 容器, 2C2G):"
echo "  scp docker-compose.prod-lite.yml .env root@你的ECS:/opt/docstamp/"
echo "  ssh root@你的ECS 'cd /opt/docstamp && docker compose -f docker-compose.prod-lite.yml pull && docker compose -f docker-compose.prod-lite.yml up -d'"
