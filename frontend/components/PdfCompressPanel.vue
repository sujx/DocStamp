    ...
    <UFormGroup v-if="file" :label="$t('pdfCompress.quality')" class="mt-4">
      <div class="flex gap-3">
        <label v-for="q in qualities" :key="q.key" class="flex items-center gap-1.5 cursor-pointer text-sm text-primary">
          <input v-model="quality" type="radio" :value="q.key" class="accent-brand-700" /> {{ q.label }}
        </label>
      </div>
    </UFormGroup>
    <div class="mt-6 flex gap-3">
      <UButton color="primary" :disabled="!file" :loading="compressing" @click="compress">
        <UIcon name="i-heroicons-arrows-pointing-in" class="w-4 h-4 mr-1.5" /> {{ compressing ? $t("common.processing") : $t("pdfCompress.compress") }}
      </UButton>
    </div>
    <div v-if="stats" class="mt-6 p-4 rounded-lg border bg-brand-soft border-default">
      <div class="text-sm space-y-1">
        <div class="text-secondary">{{ $t("pdfCompress.originalSize") }}: <strong>{{ formatSize(stats.original_size) }}</strong></div>
        <div class="text-secondary">{{ $t("pdfCompress.compressedSize") }}: <strong>{{ formatSize(stats.compressed_size) }}</strong></div>
        <div class="text-brand-700">{{ $t("pdfCompress.ratio") }}: <strong>{{ stats.ratio }}%</strong></div>
      </div>
    </div>
  </div>
</template>
...
async function compress() {
  if (!file.value) return; compressing.value = true;
  try {
