<template>
  ...
  >
    ...
    <div v-if="!fileName" class="flex flex-col items-center gap-2">
      <UIcon :name="icon" class="w-8 h-8" :style="{ color: 'var(--color-text-tertiary)' }" />
      <p class="text-sm" :style="{ color: 'var(--color-text-secondary)' }">{{ $t("common.uploadHint") }}</p>
      <p v-if="hint" class="text-xs text-pretty" :style="{ color: 'var(--color-text-tertiary)' }">{{ hint }}</p>
    ...
    <div v-else class="flex items-center gap-3 text-left">
      <UIcon :name="fileIcon" class="w-6 h-6 shrink-0" :style="{ color: 'var(--color-text-tertiary)' }" />
      <div class="flex-1 min-w-0">
        <span class="font-semibold text-sm block truncate" :style="{ color: 'var(--color-text-primary)' }">{{ fileName }}</span>
        <span v-if="fileSize" class="text-xs text-pretty" :style="{ color: 'var(--color-text-tertiary)' }">{{ formatSize(fileSize) }}</span>
      </div>
