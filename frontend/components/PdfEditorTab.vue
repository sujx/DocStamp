<template>
  <div class="pt-3">
    ...
    <p class="text-sm mb-5 text-pretty text-secondary" >{{ $t("pdfEditor.description") }}</p>
    ...
    <template v-if="mode === 'delete' && pageThumbs.length">
      <div class="mt-4">
        <p class="text-xs mb-3 text-tertiary" >{{ $t("pdfEditor.clickToSelect") }}</p>
        ...
        <p v-if="deleteSet.size" class="text-sm mt-3 text-brand-700" >
    ...
    <template v-if="mode === 'insert' && pageThumbs.length">
      <div class="mt-4 p-4 rounded-lg bg-surface" >
    ...
    <p v-if="pageCount !== null" class="text-xs mt-3 text-tertiary" >{{ $t("pdfEditor.originalPages") }}: {{ pageCount }}</p>
  </div>
</template>
