<template>
  <div class="max-w-6xl mx-auto px-6 py-8">
    <h2 class="text-2xl font-bold mb-1.5 text-balance" :style="{ color: 'var(--color-text-primary)' }">
      {{ $t("status.title") }}
    </h2>
    <p class="text-sm mb-8 text-pretty" :style="{ color: 'var(--color-text-secondary)' }">
    ...
    <div v-if="loading" class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          <div class="h-3 w-20 rounded mb-2" :style="{ backgroundColor: 'var(--color-muted)' }" />
          <div class="h-8 w-20 rounded mb-2" :style="{ backgroundColor: 'var(--color-muted)' }" />
          <div class="h-3 w-24 rounded" :style="{ backgroundColor: 'var(--color-muted)' }" />
        </div>
      </div>
      <!-- Chart skeleton row -->
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          <div class="h-4 w-28 rounded mb-4 pb-3 border-b" :style="{ borderColor: 'var(--color-border-subtle)' }">
            <div class="h-4 w-28 rounded" :style="{ backgroundColor: 'var(--color-muted)' }" />
          </div>
          <div class="h-80 rounded" :style="{ backgroundColor: 'var(--color-muted)' }" />
        </div>
      </div>
      <!-- Line chart skeleton -->
      <div
      ...
      >
        ...
        <div class="h-72 rounded" :style="{ backgroundColor: 'var(--color-muted)' }" />
      </div>
    </div>

    <!-- No data state -->
    <div v-else-if="!data || data.total === 0" class="py-20 text-center">
      <UIcon name="i-heroicons-chart-bar" class="size-12 mx-auto mb-4" :style="{ color: 'var(--color-text-tertiary)' }" />
      <p class="text-sm mb-4" :style="{ color: 'var(--color-text-secondary)' }">{{ $t("status.noData") }}</p>
      <UButton
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        ...
        >
          <p class="text-xs font-medium uppercase mb-2" :style="{ color: 'var(--color-text-tertiary)' }">
            {{ $t("status.totalCalls") }}
          </p>
          <p class="text-3xl font-bold tabular-nums" :style="{ color: 'var(--color-brand-700)' }">{{ data.total.toLocaleString() }}</p>
          <p class="text-xs mt-2" :style="{ color: 'var(--color-text-tertiary)' }">{{ $t("status.days", { n: data.days }) }}</p>
        </div>
        <div
          class="rounded-lg p-5"
          :style="{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border-default)', borderTop: '3px solid var(--color-brand-700)', boxShadow: 'var(--shadow-card)' }"
        >
          ...
          <p class="text-3xl font-bold tabular-nums" :style="{ color: 'var(--color-brand-700)' }">{{ data.unique_visitors.toLocaleString() }}</p>
          <p class="text-xs mt-2" :style="{ color: 'var(--color-text-tertiary)' }">{{ $t("status.days", { n: data.days }) }}</p>
        </div>
      </div>
